...
*)
  ...
  in1:  while (TRUE) {
          ...
          Drain(ringBuffer, sentUnacked);  \* simulates putting on in batches          
          print <<ringBuffer, "<=RB==== AFTER Drain ===SA=>", sentUnacked>>;
            
          \* } else {
            \* just do one message transfer
...
*)
\* BEGIN TRANSLATION
\* Process variable inmsg of process Inbounder at line 38 col 15 changed to inmsg_
...
Inbounder == /\ Len(sentUnacked) > 0 /\ Len(ringBuffer) < RingBufSz
             /\ PrintT(<<ringBuffer, "<=RB==== BEFORE Drain ===SA=>", sentUnacked>>)
